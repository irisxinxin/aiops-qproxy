...
# 设置环境变量（默认切换为 exec-pool，绕过 ttyd），支持外部覆写
export QPROXY_MODE=${QPROXY_MODE:-exec-pool}
export Q_BIN=${Q_BIN:-/home/ubuntu/.local/bin/q}
export QPROXY_WS_URL=${QPROXY_WS_URL:-ws://127.0.0.1:7682/ws}
# 使用 NoAuth 模式，不设置认证信息
# export QPROXY_WS_USER=demo
# export QPROXY_WS_PASS=password123
export QPROXY_WS_POOL=${QPROXY_WS_POOL:-2}
export QPROXY_CONV_ROOT=${QPROXY_CONV_ROOT:-./conversations}
export QPROXY_SOPMAP_PATH=${QPROXY_SOPMAP_PATH:-./conversations/_sopmap.json}
export QPROXY_HTTP_ADDR=${QPROXY_HTTP_ADDR:-:8080}
export QPROXY_WS_INSECURE_TLS=${QPROXY_WS_INSECURE_TLS:-0}
export QPROXY_QSTREAM_DEBUG=${QPROXY_QSTREAM_DEBUG:-1}
export QPROXY_MEMLOG_SEC=${QPROXY_MEMLOG_SEC:-30}
export QPROXY_LOG_PAYLOAD=${QPROXY_LOG_PAYLOAD:-0}
export QPROXY_CPU_PROFILE_SEC=${QPROXY_CPU_PROFILE_SEC:-1}
export QPROXY_Q_WAKE=${QPROXY_Q_WAKE:-newline}
...
echo "ENV: QPROXY_MODE=$QPROXY_MODE Q_BIN=$Q_BIN QPROXY_WS_POOL=$QPROXY_WS_POOL QPROXY_QSTREAM_DEBUG=$QPROXY_QSTREAM_DEBUG QPROXY_MEMLOG_SEC=$QPROXY_MEMLOG_SEC QPROXY_LOG_PAYLOAD=$QPROXY_LOG_PAYLOAD QPROXY_CPU_PROFILE_SEC=$QPROXY_CPU_PROFILE_SEC QPROXY_Q_WAKE=$QPROXY_Q_WAKE"
# 设置环境变量并启动服务
env \
QPROXY_WS_URL=$QPROXY_WS_URL \
QPROXY_WS_NOAUTH=1 \
QPROXY_WS_POOL=$QPROXY_WS_POOL \
QPROXY_CONV_ROOT=$QPROXY_CONV_ROOT \
QPROXY_SOPMAP_PATH=$QPROXY_SOPMAP_PATH \
QPROXY_HTTP_ADDR=$QPROXY_HTTP_ADDR \
QPROXY_WS_INSECURE_TLS=$QPROXY_WS_INSECURE_TLS \
QPROXY_PPROF=1 \
QPROXY_MEMLOG_SEC=$QPROXY_MEMLOG_SEC \
QPROXY_LOG_PAYLOAD=$QPROXY_LOG_PAYLOAD \
QPROXY_CPU_PROFILE_SEC=$QPROXY_CPU_PROFILE_SEC \
QPROXY_QSTREAM_DEBUG=$QPROXY_QSTREAM_DEBUG \
QPROXY_Q_WAKE=$QPROXY_Q_WAKE \
